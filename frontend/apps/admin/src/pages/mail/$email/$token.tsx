...
const PublicMailPage = () => {
  ...
  return (
    <div className="min-h-screen bg-layout p-24px"><Card className="mx-auto max-w-840px" title={latest.data.email}>
      ...
      <Descriptions bordered column={1}><Descriptions.Item label="主题">{mail.subject || '无主题'}</Descriptions.Item><Descriptions.Item label="发件人">{mail.sender || '-'}</Descriptions.Item><Descriptions.Item label="时间">{mail.receivedAt || '-'}</Descriptions.Item><Descriptions.Item label="验证码"><Tag color="blue">{mail.code || '未识别'}</Tag></Descriptions.Item></Descriptions>
